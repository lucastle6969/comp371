...
void TrunkA::buildConnectorElements(int segmentConnectStart,int start, int set, char lr,
...
void TrunkA::buildTrunkElements(int start, int end,
                               std::vector<GLuint>* trunkIndices, std::vector<glm::vec3>* trunkVert,
                               std::vector<glm::vec2>* trunkUVs, std::vector<glm::vec3>* trunkNorms){
    ...
    float increments = ((end - start) / trunkPoints) / 2.0;
    float   jumps = textureTrunkHeight / increments;
...
bool TrunkA::buildTrunk(float trunkDiameter, float lineSegments){
    int randomSeedValue = buildVertices(trunkDiameter, lineSegments);
    heightCount += 3;
    //make branch check
...
int TrunkA::buildVertices(float trunkDiameter, float lineSegments){
    //build points
